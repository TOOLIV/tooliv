...
const MeetingCopy = () => {
  ...
  const startScreenShare = async () => {
    ...
    const newSession = OVForScreen.initSession();
    ...
    await getTokenForScreenShare().then((token: any) => {
      ...
      newSession
        ...
        .then(() => {
          ...
          // Init a publisher passing undefined as targetElement (we don't want OpenVidu to insert a video
          // element: we will manage it on our own) and with the desired properties
          const newPublisher = OVForScreen.initPublisher(initScreenData.myScreenName, {
            audioSource: false, // The source of audio. If undefined default microphone
            // videoSource: videoDevices[0].deviceId, // The source of video. If undefined default webcam
  ...
  return (
    <MeetingContainer>
      ...
      <button onClick={deleteSessionForScreenShare}>화면공유 세션 종료</button>
      <FunctionButtons
        isAudioOn={isAudioOn}
        isVideoOn={isVideoOn}
