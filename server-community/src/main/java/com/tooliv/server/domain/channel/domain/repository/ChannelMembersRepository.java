...
public interface ChannelMembersRepository extends JpaRepository<ChannelMembers, String> {
    ...
    @Query(value = "SELECT * "
        ...
        + "INNER JOIN user u ON m.user_id = u.id "
        + "INNER JOIN channel c ON m.channel_id = c.id "
        + "WHERE c.id = :channel_id AND u.name LIKE %:keyword% AND u.user_code != 'U04' AND  u.deleted_at IS NULL "
    ...
    @Query(value = "SELECT * "
        ...
        + "INNER JOIN user u ON m.user_id = u.id "
        + "INNER JOIN channel c ON m.channel_id = c.id "
        + "WHERE c.id = :channel_id AND u.user_code != 'U04' ", nativeQuery = true)
    List<ChannelMembers> findByChannelIsNotBot(@Param("channel_id") String channelId);
