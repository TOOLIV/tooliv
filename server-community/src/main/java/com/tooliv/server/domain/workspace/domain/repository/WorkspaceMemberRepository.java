...
public interface WorkspaceMemberRepository extends JpaRepository<WorkspaceMembers, String> {
    ...
    @Query(value="SELECT * "
        ...
        + "WHERE w.id = :workspace_id  AND u.deleted_at IS NULL AND u.user_code != 'U04' "
    ...
    @Query(value="SELECT * "
        ...
        + "WHERE m.workspace_id = :workspace_id AND u.deleted_at IS NULL AND u.user_code != 'U04' AND u.name LIKE %:keyword% AND u.id NOT IN (\n"
    ...
    @Query(value="SELECT * "
        ...
        + "INNER JOIN user u ON m.user_id = u.id "
        + "INNER JOIN workspace w ON m.workspace_id = w.id "
        + "WHERE w.id = :workspace_id AND u.name LIKE %:keyword% AND u.deleted_at IS NULL AND u.user_code != 'U04' "
        + "ORDER BY u.name LIMIT :offset, 10 ", nativeQuery = true)
